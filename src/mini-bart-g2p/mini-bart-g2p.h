...
/**
 * @class MiniBartG2P
 * @brief This class is the C++ implementation of the Mini-BART G2P model from https://huggingface.co/cisco-ai/mini-bart-g2p.
 *
 * The MiniBartG2P class provides functionality to initialize the model and convert graphemes to phonemes.
 */
class MiniBartG2P {
public:
    MiniBartG2P(std::unique_ptr<ov::Core>& core_ptr, const std::filesystem::path& model_folder_path, const std::string& device= "CPU", bool use_past_ = true);
    ~MiniBartG2P() = default;

    /**
     * @brief Converts graphemes to phonemes. Ref:OVModelForSeq2SeqLM.forward in /optimum/intel/openvino/modeling_seq2seq.py
