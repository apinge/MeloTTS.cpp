...
namespace melo {
    MiniBartG2P::MiniBartG2P(std::unique_ptr<ov::Core>& core_ptr, const std::filesystem::path& model_folder_path, const std::string& device_,
        bool use_past_): device(device_),use_past(use_past_)
    {
        ...
        encoder_model = std::make_unique<ov::CompiledModel>(core_ptr->compile_model(encoder_model_path.string(),device,set_ov_config(device)));
        encoder_req = std::make_unique<ov::InferRequest>(encoder_model->create_infer_request());
        decoder_model = std::make_unique<ov::CompiledModel>(core_ptr->compile_model(decoder_model_path.string(),device, set_ov_config(device)));
        ...
        if (use_past_ && std::filesystem::exists(decoder_model_with_past_path)) {
            decoder_with_past_model = std::make_unique<ov::CompiledModel>(core_ptr->compile_model(decoder_model_with_past_path.string(), device, set_ov_config(device)));
        ...
        try {
            ...
            */
            ...
            for (;;) {
                ...
                for (int i = (logits_row - 1) * vocab_size, j = 0;i < logits_row * vocab_size&&j<vocab_size; ++i,++j) {
    ...
    */
    ...
    const std::unordered_map<int, std::string> MiniBartG2P::detokenizer = { {0,"<s>"},{2,"</s>"}, { 11, "AH0" },{13, "N"},{16, "S"},{17, "L"},{18, "T"},{19, "R"},{20, "K"},{23, "D"},{25, "IH0"},{27, "M"},{28, "Z"},{32, "ER0"},{33, "IY0"},{35, "B"},{36, "P"},{37, "EH1"},
        ...
        {66, "AA0"},{67, "ER1"},{68, "EH2"},{69, "IH2"},{70, "TH"},{71, "AY2"},{72, "AE2"},{73, "EY2"},{74, "AA2"},{75, "EH0"},{77, "AW1"},{78, "OW2"},{80, "IY2"},{81, "UW0"},{82, "AO2"},{83, "UH1"},{84, "AE0"},{86, "AO0"},{87, "AH2"},{88, "UW2"},
        {89, "AY0"},{90, "OY1"},{92, "EY0"},{93, "DH"},{94, "AW2"},{95, "ER2"},{96, "ZH"},{97, "UH2"},{98, "AW0"},{99, "UH0"},{100, "OY2"},{101, "OY0"}};
}