...
namespace melo {
    ...
    /*    void _merge_continuous_three_tones(std::vector<std::pair<std::string, std::string>>& seg);
        ...
        void _neural_sandhi(const std::string& word, const std::vector<std::string>& chinese_characters, const std::string& tag, const std::shared_ptr<cppjieba::Jieba>& jieba, std::vector<std::string>& sub_finals);
        ...
        const static std::set<std::string> numeric = { "零", "一", "二", "三", "四", "五", "六", "七", "八", "九", "十", "百", "千", "万", "亿", "兆" };
        ...
        {
            return std::all_of(chinese_characters.begin(), chinese_characters.end(), [](const std::string& ch) {return is_numeric(ch); });
        }

    }
    const static std::unordered_set<std::string> must_not_neural_tone_words = { "男子", "女子", "分子", "原子", "量子", "莲子", "石子", "瓜子", "电子", "人人", "虎虎"};
