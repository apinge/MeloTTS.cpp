...
namespace text_normalization {
    ...
    const std::vector<std::string> belowTwenty = { "", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine", "ten", "eleven", "twelve", "thirteen", "fourteen", "fifteen", "sixteen", "seventeen", "eighteen", "nineteen" };
    ...
    static std::string int_to_words(int num) {
        ...
        while (num > 0) {
            if (num % 1000) {
                result = int_to_words_helper(num % 1000) + (thousands[i].empty() ? "" : " " + thousands[i]) + (result.empty() ? "" : " " + result);
            }
            num /= 1000;
            i++;
        }
