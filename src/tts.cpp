...
namespace melo {
    ...
    std::tuple<std::vector<std::vector<float>>, std::vector<int64_t>, std::vector<int64_t>, std::vector<int64_t>> 
        ...
        try {
            auto [phones_list, tones_list, word2ph_list] = chinese_mix::_g2p_v2("编译器compiler会尽可能从函数实参function arguments推导缺失的模板实参template arguments", tokenizer);
            auto [phones_, tones_, lang_ids_,word2ph_] = chinese_mix::cleaned_text_to_sequence(phones_list, tones_list, word2ph_list);
