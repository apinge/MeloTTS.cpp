    ...
    struct WaveHeader {
        bool Validate() const {
            ...
            if (chunk_id != 0x46464952) {
                MELO_LOG(MELO_ERROR) << "Expected chunk_id RIFF. Given: 0x" << std::hex
                    << chunk_id;
                return false;
            }
            //               E V A W
            if (format != 0x45564157) {
                MELO_LOG(MELO_ERROR) << "Expected format WAVE. Given: 0x" << std::hex
                    << format;
                return false;
            }

            if (subchunk1_id != 0x20746d66) {
                MELO_LOG(MELO_ERROR) << "Expected subchunk1_id 0x20746d66. Given: 0x"
                    << std::hex << subchunk1_id;
                return false;
            }

            if (subchunk1_size != 16) {  // 16 for PCM
                MELO_LOG(MELO_ERROR) << "Expected subchunk1_size 16. Given: "
                    << subchunk1_size;
                return false;
            }

            if (audio_format != 1) {  // 1 for PCM
                MELO_LOG(MELO_ERROR) << "Expected audio_format 1. Given: "
                    << audio_format;
                return false;
            }

            if (num_channels != 1) {  // we support only single channel for now
                MELO_LOG(MELO_ERROR) << "Expected single channel. Given: "
                    << num_channels;
                return false;
            }
            ...
            if (bits_per_sample != 16) {  // we support only 16 bits per sample
                MELO_LOG(MELO_ERROR) << "Expected bits_per_sample 16. Given: "
                    << bits_per_sample;
                return false;
            }
