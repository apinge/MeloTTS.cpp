...
#define M_PI 3.14159265358979323846
    ...
    namespace internal {
        ...
        static Vectorf pad(Vectorf& x, int left, int right, const std::string& mode,
            float value) {
        ...
        static Matrixcf stft(Vectorf& x, int n_fft, int n_hop, const std::string& win,
            bool center, const std::string& mode) {
            // hanning
            Vectorf window =
                0.5 *
                (1.f - (Vectorf::LinSpaced(n_fft, 0.f, static_cast<float>(n_fft - 1)) *
                    2.f * M_PI / n_fft)
                    .array()
                    .cos());
        ...
        static Matrixf melfilter(int sr, int n_fft, int n_mels, int fmin, int fmax) {
            ...
            Vectorf fdiff =
                mel_f.segment(1, mel_f.size() - 1) - mel_f.segment(0, mel_f.size() - 1);
            Matrixf ramps = mel_f.replicate(n_f, 1).transpose().array() -
                fft_freqs.replicate(n_mels + 2, 1).array();

            Matrixf lower =
                -ramps.topRows(n_mels).array() /
                fdiff.segment(0, n_mels).transpose().replicate(1, n_f).array();
            Matrixf upper =
                ramps.bottomRows(n_mels).array() /
                fdiff.segment(1, n_mels).transpose().replicate(1, n_f).array();
            Matrixf weights =
                (lower.array() < upper.array()).select(lower, upper).cwiseMax(0);

            auto enorm =
                (2.0 / (mel_f.segment(2, n_mels) - mel_f.segment(0, n_mels)).array())
                .transpose()
                .replicate(1, n_f);
        ...
        static Matrixf dct(Matrixf& x, bool norm, int type) {
            ...
            // type 2
            Matrixf coeff =
                2 * (M_PI * xi.transpose().array() / N * (xi.array() + 0.5)).cos();
    ...
    public:
        ...
        /// \return     complex-valued matrix of short-time fourier transform
        /// coefficients.
        static std::vector<std::vector<std::complex<float>>> stft(
            std::vector<float>& x, int n_fft, int n_hop, const std::string& win,
            bool center, const std::string& mode) {
            ...
            std::vector<std::vector<std::complex<float>>> X_vector(
                X.cols(), std::vector<std::complex<float>>(X.rows(), 0));
            for (int i = 0; i < X.cols(); ++i) {
                auto& row = X_vector[i];
                Eigen::Map<Vectorcf>(row.data(), row.size()) = X.col(i);
            }

            return X_vector;
        }

        /// \brief      compute mel spectrogram similar with
        /// librosa.feature.melspectrogram \param      x             input audio
        /// signal \param      sr            sample rate of 'x' \param      n_fft
        /// length of the FFT size \param      n_hop         number of samples between
        /// successive frames \param      win           window function. currently
        /// only supports 'hann' \param      center        same as librosa \param mode
        /// pad mode. support "reflect","symmetric","edge" \param      power exponent
        /// for the magnitude melspectrogram \param      n_mels        number of mel
        /// bands \param      f_min         lowest frequency (in Hz) \param      f_max
        /// highest frequency (in Hz) \return     mel spectrogram matrix
        static std::vector<std::vector<float>> melspectrogram(
            ...
            Vectorf map_x = Eigen::Map<Vectorf>(x.data(), x.size());
            Matrixf mel = internal::melspectrogram(map_x, sr, n_fft, n_hop, win, center,
                mode, power, n_mels, fmin, fmax)
                .transpose();
            std::vector<std::vector<float>> mel_vector(
                mel.rows(), std::vector<float>(mel.cols(), 0.f));
        ...
        /// \return     mfcc matrix
        static std::vector<std::vector<float>> mfcc(
            std::vector<float>& x, int sr, int n_fft, int n_hop,
            const std::string& win, bool center, const std::string& mode, float power,
            int n_mels, int fmin, int fmax, int n_mfcc, bool norm, int type) {
            Vectorf map_x = Eigen::Map<Vectorf>(x.data(), x.size());
            Matrixf mel = internal::melspectrogram(map_x, sr, n_fft, n_hop, win, center,
                mode, power, n_mels, fmin, fmax)
                .transpose();
            ...
            std::vector<std::vector<float>> mfcc_vector(
                dct.rows(), std::vector<float>(dct.cols(), 0.f));
...
#endif  // MELO_TTS_TEXT_LIBROSA_H_
